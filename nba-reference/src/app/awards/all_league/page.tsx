...
export default function AllLeaguePage(): React.JSX.Element {
  ...
  // Group selections by season for better display
  const seasonsMap = new Map<string, Array<typeof selections[0]>>();
  selections.forEach((sel) => {
    const seasonId = sel['season_id'] as string;
    if (!seasonsMap.has(seasonId)) {
      seasonsMap.set(seasonId, []);
    }
    seasonsMap.get(seasonId)!.push(sel);
  });
  ...
  return (
    <main className="mx-auto min-h-screen max-w-6xl px-4 py-6">
      <div className="mb-6">
        ...
        <p className="text-muted mt-1">The best 15 players in the league by season</p>
      </div>
              ...
              {selections.map((sel, index) => (
                ...
                >
                  <td className={tableCellClass('left')}>
                    {sel['start_year'] as number}-{sel['end_year'] as number}
                  </td>
                  <td className={tableCellClass('left')}>
                    <span className="font-medium">{sel['team_name'] as string}</span>
                  </td>
                  <td className={tableCellClass('left')}>{sel['position'] as string}</td>
                  <td className={tableCellClass('left')}>
                    <Link
                      href={`/players/${(sel['bref_id'] as string).slice(0, 1).toLowerCase()}/${sel['bref_id']}` as Route}
                      className={tableLinkClass}
                    >
                      {sel['full_name'] as string}
                    </Link>
                  </td>
                  <td className={tableCellClass('left')}>
                    {sel['team_abbrev'] ? (
                      <Link
                        href={`/teams/${sel['team_abbrev']}` as Route}
                        className={tableLinkClass}
                      >
                        {sel['team_abbrev'] as string}
                      </Link>
                    ) : (
                      '-'
                    )}
