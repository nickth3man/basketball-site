...
import { getDPOYVotingBySeason } from '@/lib/queries/awards';

export default async function DPOYVotesPage({
  params,
}: {
  ...
  const votes = getDPOYVotingBySeason(seasonId);
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-6">
      <div className="mb-1 text-xs text-crumb">
        ...
        <Link href={'/awards/dpoy' as Route}>DPOY</Link> / {seasonId} Voting
      </div>

      <h1 className="mb-2 text-3xl font-bold">NBA DPOY Voting - {seasonId}</h1>
      <p className="mb-4 text-sm text-muted-strong">
        Full voting breakdown for the Defensive Player of the Year award.
      </p>
