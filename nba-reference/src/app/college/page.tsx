/**
 * @fileoverview College basketball homepage.
 *
 * Displays the college basketball overview with a coming-soon message since
 * college data has not yet been populated via ETL.
 *
 * @module @/app/college/page
 */
...
import {
  getCollegeTeamDirectory,
  getCollegeLatestSeasonId,
  getCollegeSeasonLeaders,
} from '@/lib/queries';
import {
  tableBodyRowClass,
  tableCellClass,
...
const ACCENT = '#735c00';
const ACCENT_BG = 'rgba(115,92,0,0.06)';

/**
 * Renders the college basketball homepage.
 *
 * @returns The college basketball homepage JSX element
 */
export default function CollegePage(): React.JSX.Element {
  const teams = getCollegeTeamDirectory();
  const latestSeason = getCollegeLatestSeasonId();
  const leaders = latestSeason != null ? getCollegeSeasonLeaders(latestSeason, 'pts', 10, 5) : [];
    ...
    <main className="mx-auto max-w-6xl px-4 py-6">
      ...
      >
        ...
        <p className="mb-4 text-sm text-muted">
          NCAA college basketball — players, teams, and season statistics.
        </p>
        <div className="flex flex-wrap gap-3 text-sm">
          <Link href={'/college/players' as Route} className={tableLinkClass}>
            Players Directory
          </Link>
          <Link href={'/college/teams' as Route} className={tableLinkClass}>
            Teams Directory
          </Link>
        </div>
      </div>
      ...
      <div className="bg-surface-container mb-8 rounded-lg border border-dashed border-muted/40 px-6 py-8 text-center">
        ...
        <p className="text-sm text-muted">
          College basketball statistics are not yet available. Data will be populated via ETL
          pipeline. Check back later for full coverage of NCAA players, teams, and season stats.
        </p>
      </div>
      ...
      <section className="mb-8">
        ...
        {leaders.length === 0 ? (
          <p className="text-sm text-muted">No college stat leaders available yet.</p>
        ) : (
      ...
      <section>
        ...
        {teams.length === 0 ? (
          <p className="text-sm text-muted">No college teams available yet.</p>
        ) : (
          <div className={tableContainerClass}>
            <table className={tableClass}>
              <thead>
                  ...
                  <tr key={team.abbreviation} className={tableBodyRowClass}>
                    <td className={tableCellClass('left')}>
                      <Link
                        ...
                        href={`/college/teams/${team.abbreviation}` as Route}
                      >
