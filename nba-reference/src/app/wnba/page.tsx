/**
 * @fileoverview WNBA league homepage.
 *
 * Displays the WNBA league overview with a coming-soon message since
 * WNBA data has not yet been populated via ETL.
 *
 * @module @/app/wnba/page
 */

import type React from 'react';
import type { Route } from 'next';
import Link from 'next/link';
import { getWnbaTeamDirectory, getWnbaLatestSeasonId, getWnbaSeasonLeaders } from '@/lib/queries';
import {
  tableBodyRowClass,
  tableCellClass,
  tableClass,
...
const ACCENT = '#00245e';
const ACCENT_BG = 'rgba(0,36,94,0.06)';

/**
 * Renders the WNBA league homepage.
 *
 * @returns The WNBA homepage JSX element
 */
export default function WnbaPage(): React.JSX.Element {
  const teams = getWnbaTeamDirectory();
  const latestSeason = getWnbaLatestSeasonId();
  const leaders = latestSeason != null ? getWnbaSeasonLeaders(latestSeason, 'pts', 10, 5) : [];
    ...
    <main className="mx-auto max-w-6xl px-4 py-6">
      ...
      >
        ...
        <p className="mb-4 text-sm text-muted">
          Women&apos;s National Basketball Association — players, teams, and season statistics.
        </p>
        <div className="flex flex-wrap gap-3 text-sm">
          <Link href={'/wnba/players' as Route} className={tableLinkClass}>
            Players Directory
          </Link>
          <Link href={'/wnba/teams' as Route} className={tableLinkClass}>
            Teams Directory
          </Link>
        </div>
      </div>
      ...
      <div className="bg-surface-container mb-8 rounded-lg border border-dashed border-muted/40 px-6 py-8 text-center">
        ...
        <p className="text-sm text-muted">
          WNBA statistics are not yet available. Data will be populated via ETL pipeline. Check back
          later for full coverage of players, teams, and season stats.
        </p>
      </div>
      ...
      <section className="mb-8">
        ...
        {leaders.length === 0 ? (
          <p className="text-sm text-muted">No WNBA stat leaders available yet.</p>
        ) : (
      ...
      <section>
        ...
        {teams.length === 0 ? (
          <p className="text-sm text-muted">No WNBA teams available yet.</p>
        ) : (
          <div className={tableContainerClass}>
            <table className={tableClass}>
              <thead>
                  ...
                  <tr key={team.abbreviation} className={tableBodyRowClass}>
                    <td className={tableCellClass('left')}>
                      <Link
                        ...
                        href={`/wnba/teams/${team.abbreviation}` as Route}
                      >
