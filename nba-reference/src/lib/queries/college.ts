/**
 * @fileoverview College basketball query functions.
 * Since the DB does not yet contain college data, these queries will return empty arrays or null.
 * All queries filter by lg = 'COLLEGE'.
 * @module @/lib/queries/college
 */
...
/**
 * Represents a row in the college basketball season leaders query result.
 */
export interface CollegeLeaderRow {
  bref_id: string;
  full_name: string;
  team: string | null;
  g: number;
...
/**
 * Retrieves a paginated list of college basketball players for the directory.
 * Returns empty array until college data is populated in the database.
 *
 * @param limit - Maximum number of players to return (default: 400)
 * @param offset - Number of records to skip (default: 0)
 * @returns Array of player directory records
 */
export function getCollegePlayerDirectory(limit = 400, offset = 0): PlayerDirectoryRow[] {
...
/**
 * Returns the total count of college basketball players, optionally filtered by first letter of bref_id.
 *
 * @param letter - Optional single letter to filter by (a-z)
 * @returns Total count of matching players
 */
export function getCollegePlayerDirectoryCount(letter?: string): number {
...
/**
 * Retrieves the complete list of college basketball teams for the directory.
 *
 * @returns Array of team directory records
 */
export function getCollegeTeamDirectory(): TeamDirectoryRow[] {
...
/**
 * Retrieves the most recent college basketball season ID from the database.
 *
 * @returns The latest season ID string, or null if no college data exists
 */
export function getCollegeLatestSeasonId(): string | null {
