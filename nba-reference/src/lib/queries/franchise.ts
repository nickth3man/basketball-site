...
export function getFranchiseSeasons(teamAbbrev: string): FranchiseSeason[] {
  return getCachedQueryMany<FranchiseSeason[]>(
    `SELECT 
      ...
      ts.wins,
      ts.losses,
      ROUND(ts.wins * 100.0 / NULLIF(ts.wins + ts.losses, 0), 1) as win_pct,
      CASE 
    ...
    JOIN dim_team t ON t.team_id = ts.team_id
    WHERE t.bref_abbrev = ? OR t.abbreviation = ?
...
export function getFranchiseChampionships(teamAbbrev: string): Championship[] {
  ...
  const rows = getCachedQueryMany<
    Array<{ season_id: string; game_date: string; year: number }>
  >(
    ...
    [teamAbbrev, teamAbbrev, teamAbbrev],
    60_000
  );
...
export function getCurrentFranchiseInfo(teamAbbrev: string): {
  ...
  return getCachedQueryOne(
    ...
    [teamAbbrev, teamAbbrev],
    60_000
  ) as { team_name: string; city: string; founded: number; conference: string | null; division: string | null } | undefined;
}
