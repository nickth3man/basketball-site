...
```
find . -not -path '*/node_modules/*' -not -path '*/.git/*' -not -path '*/venv/*' -not -path '*/__pycache__/*' -not -path '*/dist/*' -not -path '*/build/*' -not -path '*/.next/*' -not -path '*/.audit/*' | head -250
```
...
Run ALL of these pattern searches. Use `bash` with `rg` (ripgrep) for every search.
...
After completing ALL phases above, produce a single comprehensive markdown report. Write it to `.audit/plans/audit-report.md` using the write tool.
...
7. **READ-ONLY** — Do not modify, create, or delete any project files EXCEPT writing the report to `.audit/plans/audit-report.md`.
